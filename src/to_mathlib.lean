...
@[ext] theorem to_nat_inj {n} {x y : dfin n} (e : x.to_nat = y.to_nat) : x = y :=
...
{ sup := λ _ _, (),
  ...
  lt := λ _ _, false,
  le_refl := λ _, ⟨⟩,
  le_trans := λ _ _ _ _, id,
  lt_iff_le_not_le := λ _ _, ⟨false.elim, λ h, h.2 ⟨⟩⟩,
  ...
  neg := λ _, (),
  sub := λ _ _, (),
  inf_neg_eq_bot := λ _, subsingleton.elim _ _,
  sup_neg_eq_top := λ _, subsingleton.elim _ _,
  sub_eq := λ _ _, subsingleton.elim _ _,
  Sup := λ _, (),
  Inf := λ _, (),
  le_Sup := λ _ _ _, ⟨⟩,
  Sup_le := λ _ _ _, ⟨⟩,
  Inf_le := λ _ _ _, ⟨⟩,
  le_Inf := λ _ _ _, ⟨⟩,
  infi_sup_le_sup_Inf := λ _ _, ⟨⟩,
  inf_Sup_le_supr_inf := λ _ _, ⟨⟩ }
